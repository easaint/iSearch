...
Author: tashaxing
*/
#ifndef _TL_SYNC_LIST_H
#define _TL_SYNC_LIST_H

#include <list>
#include <mutex>

namespace tl_thread_safe {
...
public:
	...
	list(const tl_thread_safe::list<T, Allocator> & x) { std::lock_guard<std::mutex> lock(x.mutex); storage = x.storage; }

    // Copy
	tl_thread_safe::list<T, Allocator>& operator=(const tl_thread_safe::list<T, Allocator>& x) { std::lock_guard<std::mutex> lock(mutex); std::lock_guard<std::mutex> lock2(x.mutex); storage = x.storage; return *this; }
	...
	void swap(tl_thread_safe::list<T, Allocator> & x) { std::lock_guard<std::mutex> lock(mutex); std::lock_guard<std::mutex> lock2(x.mutex); storage.swap(x.storage); }

    void clear( void ) { std::lock_guard<std::mutex> lock( mutex ); storage.clear(); }

    // Operations
	void splice(iterator position, tl_thread_safe::list<T, Allocator>& x) { std::lock_guard<std::mutex> lock(mutex); std::lock_guard<std::mutex> lock2(x.mutex); storage.splice(position, x.storage); }
	void splice(iterator position, tl_thread_safe::list<T, Allocator>& x, iterator i) { std::lock_guard<std::mutex> lock(mutex); std::lock_guard<std::mutex> lock2(x.mutex); storage.splice(position, x.storage, i); }
	void splice(iterator position, tl_thread_safe::list<T, Allocator>& x, iterator first, iterator last) { std::lock_guard<std::mutex> lock(mutex); std::lock_guard<std::mutex> lock2(x.mutex); storage.splice(position, x.storage, first, last); }
	...
	void merge(tl_thread_safe::list<T, Allocator>& x) { std::lock_guard<std::mutex> lock(mutex); std::lock_guard<std::mutex> lock2(x.mutex); storage.merge(x.storage()); }
	template <class Compare> void merge(tl_thread_safe::list<T, Allocator>& x, Compare comp) { std::lock_guard<std::mutex> lock(mutex); std::lock_guard<std::mutex> lock2(x.mutex); storage.merge(x.storage, comp); }
